...
folder = 'C:\Users\fatadama\Documents\summer 2014\Optitrack\June 25 optic flow test';
...
delta = alignTimeHistories(t,-xyz(:,3),tv,flow(:,5),-4.04);
...
fprintf('Mean residuals in tind: %.2g,%.2g m/s\n',mean(resid(tind,1:2)))
fprintf('Std of residuals in tind: %.2g,%.2g m/s\n',std(resid(tind,1:2)));
fprintf('Mean abs flow velocity in tind: %.2g,%.2g m/s\n',mean(abs(vb_flow_in(tind,1:2))));
fprintf('Mean altitude in tind: %.2g m\n',mean(-xyz_in(tind,3)));

tt = find(tv > 6 & flow(:,6) >  min(flow(tind,6)));
for i = 1:2
    X = [ones(length(tt),1) vb_flow_in(tt,i)];
    y = resid(tt,i);
    b = regress(y,X);
    fprintf(['Slope of axis ' num2str(i) ' optic flow residuals overall w/ good flow Q: %.3g deg\n'],r2d*b(2));
end
fprintf('Mean residuals overall: %.2g,%.2g m/s\n',mean(resid(tt,1:2)))
fprintf('Std of residuals overall: %.2g,%.2g m/s\n',std(resid(tt,1:2)));
fprintf('Mean abs flow velocity overall: %.2g,%.2g m/s\n',mean(abs(vb_flow_in(tt,1:2))));
fprintf('Mean altitude overall: %.2g m\n',mean(-xyz_in(tt,3)));
...
for i = 1:2
    above = find(abs(chi2(:,i)) > thres);
    below = find(abs(chi2(:,i)) <= thres);
    
    subplot(2,1,i);
    plot(vb_flow_in(below,i),resid(below,i),'x');
    hold on
    plot(vb_flow_in(above,i),resid(above,i),'rd');
end