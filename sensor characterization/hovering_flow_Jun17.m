...
folder = 'C:\Users\fatadama\Documents\summer 2014\Optitrack\June 17 crash';
...
delta = alignTimeHistories(t(1:12924),euler(1:12924,1),tv,Att(:,1),-0.94);
...
fprintf('Mean abs x and y velocity measured (m/s): %.2g,%.2g\n',mean(abs(flowshort(:,3:4))));

% fit line to residuals versus optitrack accels
for i = 1:2
    X = [ones(length(tshort),1) flowshort(:,2+i)];
    y = resid(:,i);
    b = regress(y,X);
