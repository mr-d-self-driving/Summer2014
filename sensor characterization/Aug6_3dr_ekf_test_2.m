...
folder = 'C:\Users\fatadama\Documents\summer 2014\Optitrack\August 6 3dr ekf test 2';
%%
...
Cflow = [.7071 -.7071;.7071 .7071];
vb_flow = vb(:,1:2)*Cflow';

%% align time histories
%delta = alignTimeHistoriesPh(t,vb(:,1),tv,ekf(:,1));
delta = alignTimeHistoriesPh(t,euler(:,2),tv,ekf(:,5));
...
if exist('t_st','var')
    ...
    fprintf('Roll angle error std: %g deg\n',nanstd(ekf(intv,4)-euler_in(intv,1))*r2d);
    fprintf('Pitch angle error std: %g deg\n',nanstd(ekf(intv,5)-euler_in(intv,2))*r2d);
    fprintf('Their attitude angle error std: %g deg %g deg\n',nanstd(Att(intv,1:2)-euler_in(intv,1:2))*r2d);
    fprintf('Roll angle error mean: %g deg\n',nanmean(ekf(intv,4)-euler_in(intv,1))*r2d);
    fprintf('Pitch angle error mean: %g deg\n',nanmean(ekf(intv,5)-euler_in(intv,2))*r2d);
    fprintf('Their attitude angle error mean: %g deg %g deg\n',nanmean(Att(intv,1:2)-euler_in(intv,1:2))*r2d);
end
