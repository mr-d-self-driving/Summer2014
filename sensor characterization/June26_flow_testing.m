...
folder = 'C:\Users\fatadama\Documents\summer 2014\Optitrack\June 26 optic flow test';
...
delta = 0.24;
%delta = alignTimeHistories(t,-xyz(:,3),tv,flow(:,5),0.24);
...
%   std = .4364(z-0.5) + 0.4, 0.5 < z <= 1.875
%   std = 1, 1.875 < z
...
for i = 1:2
    ...
    ttin{i} = find( vb_flow_in(:,i) < 1*-xyz_in(:,3) );%checks true velocity < 1 m/s per m altitude
    ttin2{i} = find( abs(vb_flow_in(tind,i)) < 1 );% checks true velocity < 1
    ttin3 = find(abs(flow(:,2+i)) < 1*flow(:,5)); % checks optic flow measurements against 1 m/s per m of altitude
    ...
    X = [ones(length(ttin{i}),1) vb_flow_in(ttin{i},i)];
    y = resid(ttin{i},i);
    b = regress(y,X);
    fprintf(['Slope of axis ' num2str(i) ' ttin optic flow residuals: %.3g deg\n'],r2d*b(2));
    ...
    X = [ones(length(ttin3),1) vb_flow_in(ttin3,i)];
    y = resid(ttin3,i);
    b = regress(y,X);
    fprintf(['Slope of axis ' num2str(i) ' ttin3 optic flow residuals in tind: %.3g deg\n'],r2d*b(2));
end
...
%% mean error vs time
% how to characterize bias rate?

N = 50;

meani = zeros(length(tv)-N,2);
for i = 1:length(tv)-N
    meani(i,:) = nanmean(resid(i:(i+N),1:2));
end

figure;
plot(tv(1:end-N),meani);
figure
plot(tv(tind),meani(tind,:))