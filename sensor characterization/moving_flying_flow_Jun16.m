...
folder = 'C:\Users\fatadama\Documents\summer 2014\Optitrack\June 16 test';
...
delta = alignTimeHistories(t,vb_flow(:,1),tshort,flowshort(:,3),0.0);
...
subplot(212);
title('\sigma');
plot(tshort(tind),movingsig(tind,:));
...
slopealt = zeros(length(tshift),1);
slopex = zeros(length(tshift),1);
slopey = zeros(length(tshift),1);
for i = 1:length(tshift)
    vb_interer = interp1(tmat(:,i),vb_flow(:,1:2),tshort);
    alt_interer = interp1(tmat(:,i),-xyz(:,3),tshort);
    
    residx = vb_interer(:,1) - flowshort(:,3);
    residy = vb_interer(:,2) - flowshort(:,4);
    residalt = alt_interer - flowshort(:,5);
    
    X = [ones(length(tind),1) flowshort(tind,3)];
    y = residx(tind);
    b = regress(y,X);
    slopex(i) = b(2);
    
    X = [ones(length(tind),1) flowshort(tind,4)];
    y = residy(tind);
    b = regress(y,X);
    slopey(i) = b(2);
