...
void mavlinkClass::send_heartbeat(int *fd){
	...
	MAV_MODE_STABILIZE_ARMED,//255,
	MAV_MODE_FLAG_STABILIZE_ENABLED,//0,
	MAV_STATE_ACTIVE);
	// Copy the message to the send buffer
	uint16_t len = mavlink_msg_to_send_buffer(buffy, &msg);
	// send the message
...
uint32_t mavlinkClass::get_hb_counts(){
	return hb_counter;
}
...
void mavlinkClass::send_cmd_arm(int *fd,bool state){
	...
	mavlink_msg_command_long_pack(system_id,
		...
		&msg,
		target_system_id,
		MAV_COMP_ID_SYSTEM_CONTROL,//target_component_id,
