% no range to other agent measurement
...
meas{1}(:,1:3) = meas{1}(:,1:3)./repmat( sqrt(sum(meas{1}(:,1:3).^2,2)),1,3);
meas{2}(:,1:3) = meas{2}(:,1:3)./repmat( sqrt(sum(meas{2}(:,1:3).^2,2)),1,3);
...
% measurement error
Rx = diag([err_dev err_dev err_dev err_dev]).^2;
...
for j = 1:2
    for k = 1:length(T)-1
        ...
        yk = [atan2(rji_i(2),rji_i(1));atan2(rji_i(3),sqrt(sum(rji_i(1:2).^2))); ...
		atan2(rij_j(2),rij_j(1));atan2(rij_j(3),sqrt(sum(rij_j(1:2).^2)))];
        Pvk = Qk;
        
        Pnk = Rx;
        
        [xp,Pp] = ukf_update(xhat,Pk,Pvk,Pnk,uk,yk,@update_eq_all,@measurement_eq_no_range);
