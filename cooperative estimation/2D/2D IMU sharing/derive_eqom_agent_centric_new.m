...
syms rkix rkiy;
rki_bi = [rkix;rkiy];
rkj_bi = rki_bi - rji_bi;

delta1 = 2*atan((tan(thetaij/2)*tan(thetaji/2) + 1)/(tan(thetaij/2) - tan(thetaji/2)));
delta2 = -2*atan((tan(thetaij/2) - tan(thetaji/2))/(tan(thetaij/2)*tan(thetaji/2) + 1));

    bj2 = [-sin(delta) cos(delta)];
    bj1 = [cos(delta) sin(delta)];
    Phi = (bj2*rkj_bi)/(bj1*rkj_bi) - tan(thetakj);
    dz = [rkix;rkiy;rhoji;thetaji;thetaij;delta];
    dPhi = sym('P',[length(dz),1]);
    for i = 1:length(dz)
        dPhi(i) = simplify(diff(Phi,dz(i)));
    end