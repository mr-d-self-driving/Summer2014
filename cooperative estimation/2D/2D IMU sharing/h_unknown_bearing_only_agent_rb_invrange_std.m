% function [ytilde,yexp,Hk,Rk] = h_unknown_bearing_only_agent_rb_invrange_std()
...
Nf = xk(j).Nf;
x0hat = xk(j).xk(2*i-1,:)';
P0hat = reshape( xk(j).Pk(2*i-1,:), 4*M*N+5, 4*M*N+5 );
...
for m = 1:M
    ...
    if abs(thetaki) <= FOV(j)*d2r && abs(ags(j).rb(i,m)) < RANGE(j) && abs(ags(j).rb(i,m)) > RANGEMIN(j)
        ...
        if ismember((j-1)*M+m,xk(j).fl)
            ...
            % check numerically
%                 Hp = zeros(1,length(x0hat));
%                 for kk = 1:length(x0hat)
%                     xp = x0hat;
%                     xp(kk) = xp(kk) + 1e-10;
%                     xyi = xp(indNf+(1:2));
%                     invrhoi = xp(indNf + 3);
...
%                     rbp = xp(1:2);
%                     psip = xp(5);
%                     Cbnp = [cos(psip) sin(psip);-sin(psip) cos(psip)];
%                     % inertial frame expected vector to feature:
%                     ri = xyi + 1/invrhoi*[cos(thetai);sin(thetai)];
%                     % body frame expected vector to feature from body:
%                     rib = Cbnp*(ri-rbp);
%                     % expected measurement:
%                     yp = atan2(rib(2),rib(1));
%                     yp = minangle(yp,yexp(measurement_count));
%                     Hp(kk) = -(yexp(measurement_count)-yp).*1e10;
%                 end
%                 disp('wait');
        else
            ...
            P = zeros(indNf+2);
            P(1:indNf,1:indNf) = P0hat(1:indNf,1:indNf);
...
for j2 = [1:j-1 j+1:N]
    ...
    for m = 1:M
        ...
        if abs(ags(j2).rb(i,m+M)) <= d2r*FOV(j2) && abs(ags(j2).rb(i,m)) < RANGE(j2) && abs(ags(j2).rb(i,m)) > RANGEMIN(j2)
            ...
            if ismember((j2-1)*M+m,xk(j).fl)
                measurement_count = measurement_count + 1;
                % index of the feature state estimate in the overall state
                indfl = find(xk(j).fl == (j2-1)*M+m);
                indNf = 5+4*(indfl-1);
                % compute the expectation
                xyi = x0hat(indNf+(1:2));
                invrhoi = x0hat(indNf + 3);
