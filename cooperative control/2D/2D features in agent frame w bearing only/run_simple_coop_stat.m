        ...
        if bitand(readmask,2) || bitand(readmask,1)
            ...
            if update_counter == K_update % check if it's time to get a shared measurement
                ...
                for j2 = [1:j-1 j+1:N]
                    ...
                    %delta = atan2(sin(thetaji)-cos(thetaji)*tan(thetaij),cos(thetaji)+sin(thetaji)*tan(thetaij));
                    %2*atan((tan(thetaij/2)*tan(thetaji/2) + 1)/(tan(thetaij/2) - tan(thetaji/2)))
                    %-2*atan((tan(thetaij/2) - tan(thetaji/2))/(tan(thetaij/2)*tan(thetaji/2) + 1))
                    [pi2pi(pi-thetaij+thetaji) 2*atan((tan(thetaij/2)*tan(thetaji/2) + 1)/(tan(thetaij/2) - tan(thetaji/2))) -2*atan((tan(thetaij/2) - tan(thetaji/2))/(tan(thetaij/2)*tan(thetaji/2) + 1))]
                    ...
                    for m = 1:M
                        ...
                        if abs(ags(j2).rb(i,m+M)) <= d2r*FOV(j2) && abs(ags(j2).rb(i,m)) < RANGE(j2) && abs(ags(j2).rb(i,m)) > RANGEMIN(j2)
                            ...
                            if bitand(readmask,1)
                                ...
                                Jac(cooperative_count,[1:6 (6+cooperative_count)]) = [(-rki_bi(1)*sin(thetaji) + rki_bi(2)*cos(thetaji))/rhokj^2;
                                    (rhoji*(-rki_bi(1)*cos(thetaji)-rki_bi(2)*sin(thetaji))+rhoji^2)/rhokj^2 - 1;
                                    1;
                                    (rki_bi(2) - rhoji*sin(thetaji))/rhokj^2;
                                    -(rki_bi(1)-rhoji*cos(thetaji))/rhokj^2;
                                    [-(rki_bi(2) - rhoji*sin(thetaji)) (rki_bi(1)-rhoji*cos(thetaji))]/rhokj^2*[-sin(psii) cos(psii);-cos(psii) -sin(psii)]*[rkx;rky];
                                    -1]';
                            end
