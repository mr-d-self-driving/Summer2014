        ...
        if bitand(readmask,2) || bitand(readmask,1)
            ...
            if update_counter == K_update % check if it's time to get a shared measurement
                ...
                for j2 = [1:j-1 j+1:N]
                    ...
                    yeff = zeros(2*M+4,1);
                    yeff(1:4) = [psii;psij;rhoji;thetaji];
                    % loop over each landmark
                    for m = 1:M
                        if abs(ags(j2).rb(i,m+M)) <= d2r*FOV(j2) && abs(ags(j2).rb(i,m)) < RANGE(j2) && abs(ags(j2).rb(i,m)) > RANGEMIN(j)
                            ...
                            if bitand(readmask,2)
                                ...
                                yeff(4+cooperative_count) = rhokj;%range
                                yeff(4+cooperative_count+1) = thetakj;%bearing
                                rkx = marks(m).xy(1);
                                rky = marks(m).xy(2);
                    ...
                    if cooperative_count > 0
                        ...
                        [y,Py] = statisticalLinearization(yeff(1:4+cooperative_count),Rx(1:4+cooperative_count,1:4+cooperative_count),@coopOutputFun);
                        ...
                        Rk_jac = Jac(1:cooperative_count,1:(4+cooperative_count))*Rx(1:(4+cooperative_count),1:(4+cooperative_count))*Jac(1:cooperative_count,1:(4+cooperative_count))';
