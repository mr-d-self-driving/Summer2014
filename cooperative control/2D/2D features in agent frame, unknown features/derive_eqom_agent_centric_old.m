clear variables;
syms psii rkix rkiy rhokj rhoki rhoji u v real;
syms thetakj thetaji psij real thetaki;

rhokit = sqrt(rkix^2+rkiy^2);
...
thetaki_tilde = thetaji - atan((2*rhoji*rhokj*sin(psii - psij + thetaji - thetakj))/(rhoji^2 + rhoki^2 - rhokj^2));

dx = [u v psii rkix rkiy];
%% own agent's measurements
for i =1:length(dx)
    drdx(i,:) = simplify(diff(rhokit,dx(i)));
end

rki_b = [rkix;rkiy];%body frame expectation of relative vector
