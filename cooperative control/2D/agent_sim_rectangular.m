...
% STATUS flag
% 0 means we have just initizlied
% 1 means we are driving to +XM
% 2 means we are at +XM and rotating pi radians
% 3 means we are at +XM and translating +/- 2*SM*sin(GAM) along inertial Y
%       where SM is the maximum feature detection range and GAM is the
%       detection half-angle
% 4 means we are driving to -XM
% 5 means we are at -XM and rotating pi radians
% 6 means we are at -XM and translating -/+ along inertial Y
STATUS = 0;
...
for i = 1:length(T)
    ...
    function [v,w,a] = vel(t,y)
        switch STATUS
            case 0
                %we have just initialized, angular velocity should drive us
                %to 0 or pi radians, whichever is closer
                psi = y(3);
                if abs(pi2pi(psi)) < abs(pi2pi(psi)-pi)
                    % in this case we are closest to 0
                    w = -Kw*(psi)
                else
                    % in this case we are closest to pi
                end
        end
        a = r0/2;
        WW = pi/20;
